...
from projects.mmdet3d_plugin.models.utils.decoder_utils import ImageRCNNBlockV2, PointRCNNBlockV2, PositionEmbeddingLearned, TransformerDecoderLayer, FFN
import pdb


@HEADS.register_module()
class DeepInteractionPlusPlusDecoder(nn.Module):
    def __init__(self,
                 num_views=0,
        ...
        super(DeepInteractionPlusPlusDecoder, self).__init__()
        ...
        for i in range(int(self.num_mmpi/2)):
            ...
            self.pred_head.append(FFN(hidden_channel, heads, conv_cfg=conv_cfg, norm_cfg=norm_cfg, bias=bias))
    ...
    def forward(self, pts_inputs, img_inputs, img_metas):
        ...
        query_pos_look_forward = res_layer['center'].detach().clone()
        ...
        for layer_idx in range(self.num_mmpi):
            ...
            res_layer = self.pred_head[layer_idx](query_feat)
            pred_delta = res_layer['center'].clone()
            res_layer['center'] = pred_delta + query_pos_look_forward
            query_pos_look_forward = pred_delta + query_pos
            if layer_idx % 2 == 0:
                # self.on_the_image_mask.append(on_the_image != -1)
                self.on_the_image_mask.append(on_the_image != -1 if layer_idx == 0 else ((on_the_image != -1) & self.on_the_image_mask[-1]))
            else:
                self.on_the_image_mask.append(self.on_the_image_mask[-1])
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, preds_dicts, **kwargs):
        ...
        if hasattr(self, 'on_the_image_mask'):
            for idx_layer in range(self.num_mmpi):
                label_weights[...,idx_layer * self.num_proposals:(idx_layer + 1) * self.num_proposals] = label_weights[...,idx_layer * self.num_proposals:(idx_layer + 1) * self.num_proposals] * self.on_the_image_mask[idx_layer]
                bbox_weights[:,idx_layer * self.num_proposals:(idx_layer + 1) * self.num_proposals,:] = bbox_weights[:,idx_layer * self.num_proposals:(idx_layer + 1) * self.num_proposals,:] * self.on_the_image_mask[idx_layer][:, :, None]
